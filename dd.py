...
class Streamer:
    ...
    @property
    def introduce(self):
        return f"{self.country} 내 트위치 팔로워 <a href='{api_url}/twitch/addlogin/?logins={self.login}&skip_already_done=false&give_chance_to_hakko=true'>{self.localrank}위({self.followers}명, {tools.tdtoko(tools.now() - self.last_updated)}전 확인)</a> 인 {self.name} ({self.login})"
...
class D:
    ...
    def streamers_datas_update(cls, datas):
        ...
        to_add = [i for i in datas if i['id'] not in streamers_id]  # 이렇게 하면 중복 아이디 안 생기지 않을까
