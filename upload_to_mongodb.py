...
if not list(db.follow_data.find()):
    ...
    for i in following_data_file:
        if not i in streamers_data_file:
            ...
            temp += [{'from_id': streamers_data_file[i]['id'], 'to_id': j['id'], 'when': j['when'],
                      'last_updated': j['last_updated'], 'valid': True} for j in following_data_file[i]]
    print(len(temp))
    print(temp[0])
...
if not db.follow_data_information.find_one():
    ...
    for i in following_data_file:
        if not i in streamers_data_file:
            ...
            temp += [{'id': streamers_data_file[i]['id'], 'last_updated': tools.now(),
                      'following_num': len(following_data_file[i])}]

        else:
            temp += [{'id': streamers_data_file[i]['id'], 'last_updated': following_data_file[i][0]['last_updated'],
                      'following_num': len(following_data_file[i])}]
    print(len(temp))
    print(temp[0])
        ...
        if 'role' in streamers_data_file[i]:
            ...
            for j in roles:
                if not j in streamers_data_file:
                    ...
                    temp.append(
                        {'broadcaster_id': streamers_data_file[j]['id'], 'member_id': streamer_id, 'role': roles[j],
                         'valid': True})
